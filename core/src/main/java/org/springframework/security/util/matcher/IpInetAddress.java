...
record IpInetAddress(InetAddress address, int subnetMaskSize) {
	...
	private byte[] maskedRawAddress(InetAddress address) {
	...
	private static IpInetAddress of(String ip, int subnetMaskSize) {
		...
		Assert.state(address != null, "'ip' [%s] did not parse".formatted(ip));
		return new IpInetAddress(address, subnetMaskSize);
	}
