...
public class CsrfSample {
	...
	SecurityFilterChain example(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.csrf((csrf) -> {
			csrf.ignoringRequestMatchers("/foo/**");
			csrf.ignoringRequestMatchers("/bar/**");
			csrf.requireCsrfProtectionMatcher(this.myMatcher);
			csrf.csrfTokenRepository(this.myTokenRepo);
		});
